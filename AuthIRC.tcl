...
#	Il s'auto-invite & join les channels desir�.
#
...
set AuthIRC_My_Modes				"+iRShB-ws";		# Les modes "user" d�sir�.
set AuthIRC_Password				"botstats";			# Le mot de passe pour s'identifier � NickServ.
...
set AuthIRC_Chanserv_Invite			"1";				# S'invit� via 'msg Chanserv invite #chan' ? Valeur 1 = Oui, 0 = Non
...
bind notc - "*Mot de passe accept� - vous �tes maintenant identifi�*" AuthIRC:Invite
...
bind notc - "*Votre pseudo n'est pas enregistr�*" AuthIRC:NS:Register:Wait
...
proc AuthIRC:NS:Ghost { from keyword text } {
	putlog "AuthIRC: Nick d�j� utili�, utilisation de GHOST.";
...
proc AuthIRC:NS:Register:Wait { nick host handle text dest } {
	putlog "AuthIRC: Je vais enregistrer mon Nick � NickServ dans $::AuthIRC_TimeWaitBeforeRegNick secondes.";
	set ::AuthIRC_NS_Identified	"0";
	utimer $::AuthIRC_TimeWaitBeforeRegNick { AuthIRC:NS:Register; }
}

proc AuthIRC:NS:Register { } {
	putlog "AuthIRC: Enregistrement du nick � NickServ.";
...
proc AuthIRC:NS:Identify { } {
	putlog "AuthIRC: Je m'identifie � NickServ'";
